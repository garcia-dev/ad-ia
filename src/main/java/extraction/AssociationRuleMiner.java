 ...
 * <p>
 * The AssociationRuleMiner's class is a class generating the association rules
			...
			for (int i = 0; i < subsetsList.size(); i++)
				for (int j = i + 1; j < subsetsList.size(); j++)
					...
					if (!subsetsList.get(i).containsAll(subsetsList.get(j)) &&
						...
						/*
						 * Add the pair into the map, with its Frequency and
						 * Trust value if Trust value is higher than
						 * minimalTrust
						 */
						...
						/*
						 * Add the pair reverse into the map, with its Frequency
						 * and Trust value if Trust value is higher than
						 * minimalTrust
						 */
						trust = frequentItemsets.get(test) / frequentItemsets.get(subsetsList.get(j));
						if (trust >= minimalTrust)
							associationRuleMap.put(IntStream.range(0, variableSetList.size()).mapToObj(s -> variableSetList.get(variableSetList.size() - 1 - s)).collect(Collectors.toList()),
